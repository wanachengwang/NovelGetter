...
namespace MySql.Data.Types
{
	internal class MySqlTimeSpan : MySqlValue
	{
		...
		internal override void Serialize(PacketWriter writer, bool binary, object value, int length)
		{
			...
			{
				writer.WriteByte(8);
				writer.WriteByte((byte)((timeSpan.TotalSeconds < 0.0) ? 1 : 0));
				writer.WriteInteger((long)timeSpan.Days, 4);
				writer.WriteByte((byte)timeSpan.Hours);
				writer.WriteByte((byte)timeSpan.Minutes);
				writer.WriteByte((byte)timeSpan.Seconds);
				return;
			}
