...
namespace zlib
{
	internal class SupportClass
	{
		...
		{
			BinaryFormatter binaryFormatter = new BinaryFormatter();
			binaryFormatter.Serialize(binaryWriter.get_BaseStream(), objectToSend);
		}

		public static object Deserialize(BinaryReader binaryReader)
		{
			BinaryFormatter binaryFormatter = new BinaryFormatter();
			return binaryFormatter.Deserialize(binaryReader.get_BaseStream());
		}

		public static void WriteStackTrace(Exception throwable, TextWriter stream)
		{
			stream.Write(throwable.get_StackTrace());
			stream.Flush();
		}
	}
}
