...
namespace MySql.Data.MySqlClient
{
	internal abstract class Driver : IDisposable
	{
		...
		public bool IsTooOld
		{
			get
			{
				TimeSpan timeSpan = DateTime.get_Now().Subtract(this.creationTime);
				return this.Settings.ConnectionLifetime != 0 && timeSpan.get_TotalSeconds() > (double)this.Settings.ConnectionLifetime;
			}
		}
		...
		{
			this.creationTime = DateTime.get_Now();
		}
		...
		public virtual void Configure(MySqlConnection connection)
		{
			...
			if (this.serverProps == null)
			{
				...
				try
				{
					...
					{
						this.serverProps.set_Item(mySqlDataReader.GetValue(0), mySqlDataReader.GetString(1));
					}
		...
		private void LoadCharacterSets()
		{
			...
			try
			{
				...
				while (mySqlDataReader.Read())
				{
					this.charSets.set_Item(Convert.ToInt32(mySqlDataReader["id"], NumberFormatInfo.InvariantInfo), mySqlDataReader.GetString(mySqlDataReader.GetOrdinal("charset")));
				}
			}
			catch (Exception ex)
			{
