...
namespace MySql.Data.MySqlClient
{
	internal class Crypt
	{
		...
		public static byte[] Get410Password(string password, byte[] seedBytes)
		{
			...
			byte[] bytes = Encoding.get_Default().GetBytes(password);
			byte[] array = sHA.ComputeHash(bytes);
			CryptoStream cryptoStream = new CryptoStream(Stream.Null, sHA, 1);
		...
		public static byte[] GetOld410Password(string password, byte[] seedBytes)
		{
			...
			string text = Encoding.get_Default().GetString(array3).Substring(0, 8);
			...
			for (int k = 0; k < 8; k++)
			{
				byte[] array7;
				IntPtr intPtr;
				(array7 = array6)[(int)(intPtr = (IntPtr)k)] = (array7[(int)intPtr] ^ b);
			}
			return array6;
		}
		...
		public static byte[] Get411Password(string password, string seed)
		{
			...
			SHA1 sHA = new SHA1CryptoServiceProvider();
			byte[] array = sHA.ComputeHash(Encoding.get_Default().GetBytes(password));
			byte[] array2 = sHA.ComputeHash(array);
			byte[] bytes = Encoding.get_Default().GetBytes(seed);
			CryptoStream cryptoStream = new CryptoStream(Stream.Null, sHA, 1);
			cryptoStream.Write(bytes, 0, bytes.Length);
			cryptoStream.Write(array2, 0, array2.Length);
			cryptoStream.Close();
			byte[] array3 = new byte[sHA.get_Hash().Length + 1];
			array3[0] = 20;
			sHA.get_Hash().CopyTo(array3, 1);
			...
			if (password != null && password.Length != 0)
			{
				...
				if (new_ver)
				{
					...
					for (int j = 0; j < array3.Length; j++)
					{
						char[] array4;
						IntPtr intPtr;
						(array4 = array3)[(int)(intPtr = (IntPtr)j)] = (array4[(int)intPtr] ^ c);
					}
				}
